...
private:
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene)
    {
        ...
        for(unsigned int i = 0; i < mesh->mNumVertices; i++)
        {
            ...
            {
                glm::vec2 vec;
                // a Mesh::Vertex can contain up to 8 different texture coordinates. We thus make the assumption that we won't 
                // use models where a Mesh::Vertex can have multiple texture coordinates so we always take the first set (0).
        ...
        // now wak through each of the mesh's faces (a face is a mesh its triangle) and retrieve the corresponding Mesh::Vertex indices.
    ...
    vector<Mesh::Texture> loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName)
    {
        ...
        for(unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            {   // if texture hasn't been loaded already, load it
                ...
                Logger::log(1,"Texture.id: {}\npath: {}",texture.id,texture.path);
...
unsigned int TextureFromFile(const char *path, const string &directory, bool gamma)
{
    ...
    else
    {
        ...
        std::cout<<"Dict: "<<directory<<std::endl;
        stbi_image_free(data);
    }
